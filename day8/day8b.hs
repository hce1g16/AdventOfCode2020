...
main = do
  ...
  let permutations = permutateProg program 0
...
permutateProg :: [(String, Int)] -> Int -> [[(String, Int)]]
permutateProg prog p
  | p == length prog = []
  | op == "nop" = replaceVal p ("jmp", int) prog : permutateProg prog (p + 1)
  | op == "jmp" = replaceVal p ("nop", int) prog : permutateProg prog (p + 1)
  | op == "acc" = permutateProg prog (p + 1)
  where
    (op, int) = prog !! p

-- Returns (True, acc value at loop) if the program loops, (false, acc value at end) otherwise
doesLoop :: [(String, Int)] -> [Int] -> Int -> Int -> (Bool, Int)
