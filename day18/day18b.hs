...
compute xs = n
  where
    Number n = head $ doMuls $ doAdds $ parse xs

-- Evaluate any add operations in the op list
doAdds :: [Operator] -> [Operator]
doAdds [n] = [n]
doAdds (n : operator : n2 : xs) = case (n, operator, n2) of
  (Number n, Multiply, Number n2) -> Number n : operator : doAdds (Number n2 : xs)
  (Number n, Add, Number n2) -> doAdds (Number (n + n2) : xs)

-- Evaluate any multiply operations in the op list
doMuls :: [Operator] -> [Operator]
doMuls [n] = [n]
doMuls (n : operator : n2 : xs) = case (n, operator, n2) of
  (Number n, Multiply, Number n2) -> doMuls (Number (n * n2) : xs)
